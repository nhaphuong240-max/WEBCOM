...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (page.template_key && page.template_key !== 'blog_post') {
    notFound();
  }
  ...
  return (
    ...
    >
      <div style={{ padding: '12px clamp(14px, 3vw, 48px) 0' }}>
        <Link href="/blog" style={{ fontSize: 13, color: accent, textDecoration: 'none' }}>
          ← Blog
        </Link>
      </div>
